...
@section('content')
        ...
        <div class="row gy-4">
            ...
            <div class="col-md-5">
                <div class="card p-3">
                    ...
                    <a class="btn btn-outline-danger btn-sm mb-3"
                        href="{{ route('product.add-to-favourite', ['product_id' => $product->id]) }}"><i
                            class="fas fa-heart me-1"></i>افزودن به علاقه‌مندی</a>
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane fade" id="comments">
                    ...
                    @foreach ($comments as $comment)
                            ...
                            <div class="border rounded p-3 mb-2">
                                ...
                                @foreach ($comments->where('parent_id', $comment->id) as $reply)
                                    <div class="ms-4 mt-2 border-start ps-3">
                                        <small
                                            class="text-muted">{{ \Morilog\Jalali\Jalalian::forge($reply->created_at)->format('%A, %d %B %y') }}</small>
                                        <p class="mb-0"><strong>{{ $reply->user->name }}</strong>: {{ $reply->body }}
                                        </p>
                                    </div>
                                @endforeach
                                ...
                                <div class="d-flex justify-content-end  mt-3">
                                    <button class="btn btn-sm btn-outline-success" data-bs-toggle="modal"
                                        ...
                                        <i class="fas fa-plus"></i> افزودن پاسخ
                                    </button>
                                </div>
                        ...
                        @endif
                        <!-- Reply Comment Modal -->
                        <div class="modal fade" id="replyComment{{ $comment->id }}" tabindex="-1" aria-labelledby="commentModalLabel"
                            aria-hidden="true">
                            <div class="modal-dialog">
                                <form method="POST" id="create-comment-form"
                                    action="{{ route('comment.create', [$product, $comment]) }}">
                                    @csrf
    ...
    <div class="modal fade" id="addComment" tabindex="-1" aria-labelledby="commentModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <form method="POST" id="create-comment-form" action="{{ route('comment.create', $product) }}">
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">افزودن دیدگاه</h5>
